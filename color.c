...
int check_gen_confilts(int* gen){
    int errors = 0;
    for(int i = 0;i<nb_sommets;i++){
        if(!check_gen_node(gen,i)) errors++;
    }
    return errors;
}
...
int randomize(int cMax, float T){
    int s = rand()/(RAND_MAX/(nb_sommets));
    int color = rand()/(RAND_MAX/(cMax+1));
    int old_color = couleurs[s];
    int tmp = check_gen_node(couleurs,s);
    couleurs[s] = color+1;
    int delta = 1*(tmp-check_gen_node(couleurs,s));
    if (color+5>=cMax) delta += 1;
    if(delta>0){
        
        float p = rand()/((float)(RAND_MAX));
        if(p>exp(-((float)(delta))/T)){
            //if(delta>0) printf("*Conflits*\n");
            couleurs[s] = old_color;
        }
    }
}
...
void cross(int** gens,int id, int id1, int id2, float random, int maxColor){
    int* result = malloc(sizeof(int)*nb_sommets);
    int cut = rand()/(RAND_MAX/(nb_sommets));
    for(int i = 0;i<cut;i++){
        if(rand()/(RAND_MAX)<random){
            gens[id][i] = rand()/(RAND_MAX/(maxColor));
        }else{
...
void quickSort(int** array2D,int* array, int leftIndex, int rightIndex)
{
    ...
    for(int i = leftIndex; i <= rightIndex; i++)
    {
        if(array[i] <= pivot)
        {
            ...
            memcpy(array2D[pointer],memory2D,nb_sommets*sizeof(int));
            memcpy(array2D[i],array2D[pointer],nb_sommets*sizeof(int));
            memcpy(memory2D,array2D[i],nb_sommets*sizeof(int));
...
void genetic(int pop, int sel, int maxIter){
    ...
    for(int i = 0;i<pop;i++){
        ...
        random_gen(gens[i],300);
        scores[i] = check_gen_confilts(gens[i]);
    }
    quickSort(gens,scores,0,pop-1);
    
    
    for(int iter = 0;iter<maxIter;iter++){
        int couleur_max = check_gen_colors(gens[0]);
        for(int i = sel;i<pop;i++){
            int id1 = rand()/(RAND_MAX/(sel));
            int id2 = rand()/(RAND_MAX/(sel));
            cross(gens,i,id1,id2,0.1f,300);
            scores[i] = check_gen_confilts(gens[i]);
            printf("Couleurs -> %d\n",check_gen_colors(gens[0]));
        }
        quickSort(gens,scores,0,pop-1);
        printf("GENETIC : Conflits : %d , Couleurs : %d\n",check_gen_confilts(gens[0]), check_gen_colors(gens[0]));
